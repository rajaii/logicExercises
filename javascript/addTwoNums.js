...
var addTwoNumbers = function(l1, l2) {
  ...
  for (let i = 0; i < toLoop.length; i++) {
      if (!notLooped[i]) {
          if (toLoop[i] + extra >= 10) {
              sum.push((toLoop[i] + extra).toString()[1]); 
              extra = 1;
              continue;
          } else {
              sum.push(toLoop[i] + extra);
              extra = 0;
              continue;
          }
      }

      if (toLoop[i] + notLooped[i] + extra >= 10) {
          sum.push((toLoop[i] + notLooped[i] + extra).toString()[1]);
          extra = 1;
          continue;
      } else {
          sum.push((toLoop[i] + notLooped[i] + extra).toString());
          extra = 0;
      }
  }
  if (extra) {
